...
const std::vector<LocationBlock*>& ServerConfiguration::getRoutes() const {
    return _routes;
}
...
HttpResponseStatus stringToStatusCode(const std::string& statusCodeString) {
    if (statusCodeString == "200") return HTTP_OK;
    if (statusCodeString == "201") return HTTP_CREATED;
    if (statusCodeString == "204") return HTTP_NO_CONTENT;
    if (statusCodeString == "302") return HTTP_FOUND;
    if (statusCodeString == "400") return HTTP_BAD_REQUEST;
    if (statusCodeString == "401") return HTTP_UNAUTHORIZED;
    if (statusCodeString == "403") return HTTP_FORBIDDEN;
    if (statusCodeString == "404") return HTTP_NOT_FOUND;
    if (statusCodeString == "411") return HTTP_LENGTH_REQUIRED;
    if (statusCodeString == "413") return HTTP_PAYLOAD_TOO_LARGE;
    if (statusCodeString == "415") return HTTP_UNSUPPORTED_MEDIA_TYPE;
    if (statusCodeString == "500") return HTTP_INTERNAL_SERVER_ERROR;
...
void ServerConfiguration::setErrorPage(const std::string& statusCodeString, const std::string& filePath) {
    HttpResponseStatus code = stringToStatusCode(statusCodeString);
...
std::string ServerConfiguration::fetchErrorPagePath(HttpResponseStatus statusCode) const {
    std::map<HttpResponseStatus, std::string>::const_iterator it = _errorPagePaths.find(statusCode);
